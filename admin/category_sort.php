...
$last_cate = sql_fetch("select cate_order from `categorys` where cate_type = '{$cate_type}' and parent_ca_id = 0 order by cate_order desc limit 0, 1");
...
if($sort_type == "up"){
	...
	$sql = "update `categorys` set cate_order = {$order} where cate_order = '{$cate_order}' and cate_type = '{$cate_type}' and parent_ca_id = 0";
