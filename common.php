...
ini_set("session.cache_expire", 2400); // 세션 캐쉬 보관시간 (분)
...
if($app_mb_id){
    ...
    //로그인 기록
    // 현재접속자 처리
    $tmp_sql = " select count(*) as cnt from {$g5['login_table']} where lo_device = '{$device}' and lo_device_id = '{$mac}' and mb_id='{$app_mb_id}'";
    $tmp_row = sql_fetch($tmp_sql);

    if ($tmp_row['cnt']) {
        $tmp_sql = " update {$g5['login_table']} set mb_id = '{$member['mb_id']}', lo_datetime = '".G5_TIME_YMDHIS."', lo_location = '{$g5['lo_location']}', lo_url = '{$g5['lo_url']}' where lo_device = '{$device}' and lo_device_id = '{$mac}' and mb_id='{$app_mb_id}'";
...
$wishres = sql_query("select * from `wish_product` where mb_id = '{$member['mb_id']}'");
while($row=sql_fetch_array($wishres)){
	$wished[]["pd_id"] = $row["pd_id"];
}
...
//$html_process->app = $app;
//$html_process->app2 = $app2;
//$html_process->device = $device;
//$html_process->mac = $mac;



include_once (G5_MOBILE_PATH."/page/searchClass/search.php");
