...
for($i=0;$i<count($mAgent);$i++) {
    if (strpos(strtolower($_SERVER['HTTP_USER_AGENT']), $mAgent[$i])) {
        $ios = true;
        break;
    } else {
        $ios = false;
...
if($appLink){?>
    <script>
        <?php if($app){?>
        location.href='<?php echo $appLink;?>';
        <?php }?>
        <?php if($ios){?>
        setTimeout( function () {
            if ( ( new Date() ).getTime() - visitedAt < 2000 ) {
                location.href = "https://itunes.apple.com/app/com.foureight";
            }
        } ,500 );
        setTimeout( function () {
            location.href = "<?php echo str_replace("action","",$appLink);?>";
        } ,0 );
        <?php } ?>
    </script>
<?php }
...
<div id="head">
	...
	<div class="top_header <?php if($_SESSION["type1"]=="" || $_SESSION["type1"]==2){echo "bg2";}?>" onclick="location.href='<?php echo G5_URL?>';">
		<div class="owl-carousel" id="helps">
			<?php for($i=0;$i<count($help);$i++){?>
            <div class="item"><a href="<?php echo G5_BBS_URL?>/board.php?bo_table=help&wr_id=<?php echo $help[$i]["wr_id"];?>"><?php echo cut_str($help[$i]["wr_subject"],54 ,"...");?></a></div>
			<?php }?>
		</div>
	</div>
    <script>
    ...
    <form action="./" method="get" name="simplesearch" id="simplesearch" >
        ...
        <div class="search_setting <?php if($set_type==2){?>bg2<?php }?>">
            ...
            <input type="hidden" value="<?php echo ($scPriceFrom==0)?0:$scPriceFrom;?>" id="sc_priceFrom" name="priceFrom">
            <input type="hidden" value="<?php echo ($scPriceTo==0)?500000:$scPriceTo; ?>" id="sc_priceTo" name="priceTo">
            <input type="hidden" value="0" id="price_set" name="price_set">
            <input type="hidden" value="<?php echo ($order_sort)?$order_sort:"pd_loc,pd_price,pd_hits,pd_recom";?>" name="order_sort" id="order_sort">
            <input type="hidden" value="<?php if($order_sort_active){echo $order_sort_active;}else if(!$order_sort_active){echo "1,1,0,0";};?>" name="order_sort_active" id="order_sort_active">
            ...
            <input type="hidden" value="<?php echo $mb_level;?>" name="mb_level" id="mb_level">
            <div style="background-color:#f4f4f4 ">
                <div class="sch_top">
                    <input type="button" value="<?php if($cate && $cate2){echo $cate." > ".$cate2; }else{ ?>카테고리선택<?php }?>" class="sch_btn" onclick="fnwrite2();">
                    <a href="javascript:fnsuggestion('1','search');">카테고리 제안</a>
                </div>
                <div class="types sch_mid">
                    <label class="radio_tag" for="four">
                        <input type="radio" name="type2" id="four" value="8" <?php if($type2=="8" || $type2 == ""){?>checked<?php  }?> onclick="">
                ...
                <!--<div class="align_first">최신순</div>-->
                <div class="fixed_sch_ord">
                    <span class="fixed_od">최신순</span>
                </div>
                <div class="sch_ord" id="sc_sorts">
                    <?php if(count($order_item)==0){?>
                        ...
                        <?php }else{?>
                            ...
                            <label class="align first" id="sortable" for="pd_loc" >
                                <input type="checkbox" name="orders[]" value="pd_loc" id="pd_loc" >
                                <span class="round">거리순</span>
                            </label>
                        <?php }?>
                ...
                <div class="sch_price">
                    <div class="pr">
                        <h2>금액설정</h2>
                        <!--<p class="price" id="schp" onclick="fnPriceSet();"></p>  // 가격상세설정-->
                        <p class="price" id="schp" onclick="fnPriceSet();"></p>
                    </div>
                ...
                <div class="sch_btn_group">
                    ...
                    <input type="button" value="삽니다 간편등록" class="sch_btn sch_save_write" onClick="<?php if($is_member){?>fnWrite();<?php }else{?>location.href=g5_bbs_url+'/login.php';<?php }?>" <?php if($type2=="8" || $type2==""){?>style="display:inline-block"<?php }else{?>style="display:none;"<?php }?>>
                </div>
                <div class="search_close" onclick="<?php if(count($list)!=0){?>fnSetting2()<?php }else{?>fnSetting()<?php }?>" >
        ...
        <!--<span class="search_setting_bg"></span>-->
    </form>
	<div class="mobile_menu" id="mobile_menu">
		...
		<div class="menu">
            ...
            <div class="user_box <?php if($set_type==2){?>bg2<?php }?>">
				...
				--><?php /*} */?>
				<div class="profiles" onclick="location.href='<?php echo G5_MOBILE_URL?>/page/mypage/mypage.php'" style="position:relative;">
                    <div style="width:15.8vw;height:15.8vw;background-image:url('<?php if($member["mb_id"] && $member["mb_profile"]){ echo G5_DATA_URL."/profile/".$member["mb_no"]."/".$file; }else{?><?php echo G5_IMG_URL?>/no-profile.svg<?php }?>');background-size:cover;background-position:center;background-repeat:no-repeat;border:0.8vw solid #fff;border-radius: 50% 50%;">
                        <?php if($member["mb_id"] && $member["mb_profile"]){?>
                        <img src="<?php echo G5_DATA_URL."/profile/".$member["mb_no"]."/".$file;?>" alt="" class="user_profile" style="display:none" >
                        <?php }else{?>
			...
			<ul class="menu">
				...
				<li class="menu7" onclick="location.href='<?php echo G5_URL?>/mobile/page/first/'"><a href="#"><img src="<?php echo G5_IMG_URL?>/ic_menu_help.svg" alt="">도움말</a></li>
				<!--<li class="menu7" onclick="alert('안내준비중..')"><a href="#"><img src="<?php /*echo G5_IMG_URL*/?>/ic_menu_help.svg" alt="">도움말</a></li>-->
	...
	<div class="category_menu">
		...
		<div class="category catetype1">
			<ul>
				...
				<li class="cate<?php echo $category1[$i]["ca_id"]; ?> <?php if($i==0){?>active<?php }?>" id="scate<?php echo $category1[$i]["ca_id"]; ?>"><?php if($category1[$i]["icon"]){?><img src="<?php echo G5_DATA_URL."/cate/".$category1[$i][icon]; ?>" alt=""><?php }?><?php echo $category1[$i]["cate_name"];?></li>
				<?php } ?>
                <li class="sugg" onclick="fnsuggestion('');"><img src="<?php echo G5_IMG_URL?>/ic_menu_help.svg" alt="">카테고리 제안</li>
			</ul>
		</div>
	...
	<div class="category_menu2">
		...
		<div class="category catetype2">
			<ul>
				...
				<li class="cate<?php echo $category2[$i]["ca_id"]; ?> <?php if($i==0){?>active<?php }?>" id="scate<?php echo $category2[$i]["ca_id"]; ?>"><img src="<?php echo G5_DATA_URL."/cate/".$category2[$i][icon]; ?>" alt=""><?php echo $category2[$i]["cate_name"];?></li>
				<?php } ?>
                <li class="sugg" onclick="fnsuggestion('');"><img src="<?php echo G5_IMG_URL?>/ic_menu_help.svg" alt="">카테고리 제안</li>
			</ul>
		</div>
    ...
    <div class="category_menu3">
        ...
        <div class="category catetype1">
            <ul>
                <li class="cate00000 active" id="scate00000" ><img src="<?php echo G5_IMG_URL?>/ic_cate_all.svg" alt="">전체</li>
                <?php for($i=0;$i<count($category1);$i++){ ?>
                    <li class="cate<?php echo $category1[$i]["ca_id"]; ?> " id="scate<?php echo $category1[$i]["ca_id"]; ?>"><img src="<?php echo G5_DATA_URL."/cate/".$category1[$i][icon]; ?>" alt=""><?php echo $category1[$i]["cate_name"];?></li>
                <?php } ?>
                <li class="sugg" onclick="fnsuggestion('','search');"><img src="<?php echo G5_IMG_URL?>/ic_menu_help.svg" alt="">카테고리 제안</li>
            </ul>
        </div>
    ...
    <div class="category_menu4">
        ...
        <div class="category catetype2">
            <ul>
                <li class="cate000000 active" id="scate000000" ><img src="<?php echo G5_IMG_URL?>/ic_cate_all.svg" alt="">전체</li>
                <?php for($i=0;$i<count($category2);$i++){ ?>
                    <li class="cate<?php echo $category2[$i]["ca_id"]; ?> " id="scate<?php echo $category2[$i]["ca_id"]; ?>"><img src="<?php echo G5_DATA_URL."/cate/".$category2[$i][icon]; ?>" alt=""><?php echo $category2[$i]["cate_name"];?></li>
                <?php } ?>
            </ul>
        </div>
        ...
        chksearch = setTimeout(function(){
            $.ajax({
                ...
                if(data.popular != null){
                    if(data.popular.length > 0){
                        for(var i = 0 ; i < data.popular.length; i++){
                            if(data.popular[i].pp_word == "") continue;
                            popular += '<li onclick="fnPapularClick(\''+data.popular[i].pp_word+'\')">'+(i+1)+'<span>'+data.popular[i].pp_word+'</span></li>';
                        }
                    }
                    $(".search_popular ul").html(popular);
                }else{
                ...
                if(data.recent != null) {
                    if (data.recent.length > 0) {
                        for (var i = 0; i < data.recent.length; i++) {
                            if(data.recent[i].pp_word == "") continue;
                            recent += '<li onclick="fnRecentClick(\''+data.recent[i].pp_word+'\')"><span>' + data.recent[i].pp_word + '</span></li>';
                        }
                    }
                    $(".search_recent ul").html(recent);
                }else{
...
$(function(){
    ...
    /*$(document).on("click",".search_popular li , .search_recent li",function(){
        if(!$(this).hasClass("no-list")){
            var stx = $(this).children("span").text();
            alert(stx);
    ...
    $("#mobile_header #mobile_menu_btn").click(function(){
        if($(".search_setting").attr("id") == "menuon"){
            ...
            $(".search_setting_bg").removeClass("active");
        }
        location.hash = "#menu";
    });
	...
	$(".schtype .slider").click(function(){
		...
		if($(this).prev().prop("checked") == true){
            ...
            $(".write_btn2 img").attr("src","<?php echo G5_IMG_URL;?>/ic_write_btn.svg");
            ...
            $(".write_btn2 img").attr("src","<?php echo G5_IMG_URL;?>/ic_write_btn_2.svg");
            $("#theme-color").attr("content","#000000");
            $("#wr_price").attr("placeholder","판매금액");
...
*/
	...
	$(".radio_tag").click(function(){
        if($("#workcnt").prop("checked") != true && $("#worktime").prop("checked") != true && $("#workday").prop("checked")!=true){
            alert("하나 이상은 선택해야합니다.");
            return false;
        }
    ...
    $(".align").click(function(){
        ...
        if(chkchk==0){
            alert("하나 이상은 선택해야 합니다. \r\n최신순은 1순위 고정값으로 설정되어 있습니다.\r\n따라서 가격순을 제일 앞에 정렬하시면, 최신 자료 중에 가격이 저렴한 것을 계산해서 출력합니다.\r\n다른 정렬순을 추가하면 추가로 계산해서 출력합니다.");
...
function priceSet(){
    ...
    var pd_type2 = $("#set_type2").val();

    var sc_id = $("#sc_id").val();
    var sc_priceFrom = $("#sc_priceFrom").val();
    var sc_priceTo = $("#sc_priceTo").val();
    $.ajax({
        url:g5_url+"/mobile/page/ajax/ajax.category_minmax.php",
        type:"POST",
        dataType:"json",
        data:{cate2:cate2,cate1:cate1,pd_type:pd_type,pd_type2:pd_type2}
    }).done(function(data){
        console.log(data);
        if(data.max != null) {
            if(data.max==0){
                ...
                if(Number(sc_priceTo) > 500000){
                    max_price = sc_priceTo;
                }
                $("#sc_priceTo").val(max_price);
                $("#schp").text('<?php echo number_format(0);?>' + " ~ " + number_format(max_price));
                slider = $( "#slider-range" ).slider({
                    range: true,
                    min: 0,
                    max: max_price,
                ...
                if(sc_id){
                    //alert("검색값있음");
                    var max_price = data.max;
                    if(Number(sc_priceTo) > data.max){
                        max_price = sc_priceTo;
                    }
                    $("#sc_priceTo").val(sc_priceTo);
                    $("#schp").text(number_format(sc_priceFrom) + " ~ " + number_format(sc_priceTo));
                    slider = $( "#slider-range" ).slider({
                        range: true,
                        min: 0,
                        max: data.max,
                        values: [sc_priceFrom , sc_priceTo],
                        step:1000,
                        slide: function( event, ui ) {
                            if(Number(ui.values[1]) == data.max){
                                $("#price_set").val(0);
                            }else {
                                $("#price_set").val(1);
                            }
            ...
            if(sc_id) {
                ...
                slider = $( "#slider-range" ).slider({
                    ...
                    step:1000,
                    slide: function( event, ui ) {
                        if(Number(ui.values[1]) == max_price){
                            $("#price_set").val(0);
                        }else {
                            $("#price_set").val(1);
                        }
