        ...
        <div class="search_setting <?php if($set_type==2){?>bg2<?php }?>">
            ...
            <div style="background-color:#f4f4f4 ">
                <div class="sch_top">
                    ...
                    <a href="javascript:fnsuggestion('1');">카테고리 제안</a>
                </div>
...
<script>
    ...
    function fnKeyword(){
        ...
        if(window.event.keyCode != 13) {
            if (fnc == true) {setTimeout(fnkeywordon, 1500);}
        }else {//엔터 일때
            if ($("#set").val() == "2") {
                $("#sch_text").val($("#stx").val());// 설정에서 검색어 체크
                list_search();// 설정된 검색어 저장후 검색
                return false;
            } else { document.simplesearch.submit();//바로 간단검색}
        }
    }
