...
if($stx){
	...
	sql_query($sql);
	if($sctype!="research") {
        $sql = "insert into `my_search_list` set sc_type = '{$type}', sc_type2='', sc_cate1 = '', sc_cate2 = '', sc_tag = '{$stx}', mb_id = '{$mb_id}', sc_datetime = now()";
        sql_query($sql);
    }else{
        $sql = "update `my_search_list` set sc_datetime = now() where sc_id = '{$sc_id}'";
        sql_query($sql);
    }
}
...
<!-- 모바일 헤더 시작 -->
<div id="head">
	<div class="top_header" onclick="location.href='<?php echo G5_URL?>';">
		...
		<div class="search">
			<form action="./" method="get" name="simplesearch" id="simplesearch" >
                ...
                <label class="switch schtype" >
                    <input type="checkbox" id="type1" name="type1" <?php if($sc_id){if($schopt["sc_type"]==2){?>checked<?php } }?>>
                    <span class="slider round" <?php if($sc_id){if($schopt["sc_type"]==2){?>style="text-align:left;"<?php } }?> ><?php if($sc_id){if($schopt["sc_type"]==2){?>능력<?php }else{?>물건<?php } }else{?>물건<?php }?></span>
                </label>
			</form>
		</div>
	...
	<div class="search_setting">
		<form action="./" method="post" name="savesch" id="savesch" >
			<input type="hidden" name="set_type" id="set_type" value="<?php if($schopt["type"]!=""){echo $schopt["type"];}else{echo "1";}?>" >
			<input type="hidden" value="<?php if($schopt["priceFrom"]!=""){echo $schopt['priceFrom'];}?>" id="priceFrom" name="priceFrom">
			<input type="hidden" value="<?php if($schopt["priceTo"]!=""){echo $schopt['priceTo'];}?>" id="priceTo" name="priceTo">
			...
			<div class="sch_top">
				<select name="cate" id="cate" class="sel_cate input01s" required >
					...
					<option value="<?php echo $category1[$i]["cate_name"];?>" id="<?php echo $category1[$i]["ca_id"];?>" <?php if($sc_id){if($schopt["sc_cate1"] == $category1["cate_name"]){?>selected<?php } }?>><?php echo $category1[$i]["cate_name"];?></option>
					<?php } ?>
				</select>
			...
			<div class="types sch_mid">
				<label class="radio_tag" for="four">
					<input type="radio" name="type2" id="four" value="8" <?php if($sc_id){ if($schopt["sc_type2"]=="8" || $schopt["sc_type2"] == "0"){ ?>checked<?php } }else{?>checked<?php } ?>>
					<span class="slider2 round"><?php echo $sc_id;?>팝니다</span>
				</label>
				<label class="radio_tag" for="eight">
					<input type="radio" name="type2" id="eight" value="4" <?php if($sc_id){if($schopt["sc_type2"]=="4"){?>checked<?php } } ?>>
			...
			<div class="sch_btn_group">
				...
				<input type="button" value="설정저장" class="sch_btn" onclick="fnSaveSch()">
...
<!--					<input type="checkbox" name="paplur" id="paplur" value="1">-->
<!--					<span class="slider round">최신</span>-->
<!--				</label>-->
<!--				<label class="switch list" for="list_type">-->
<!--					<input type="checkbox" name="list_type" id="list_type">-->
<!--					<span class="slider round"></span>-->
<!--				</label>-->
<!--			</div>-->
<!--		</div>-->
