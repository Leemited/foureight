...
/*if($member["mb_profile"]){
    $filename = basename($member["mb_profile"]);
    $dirname = dirname($member["mb_profile"]);
    if (!is_dir($dirname)) {
        @mkdir($dirname, G5_DIR_PERMISSION);
        @chmod($dirname, G5_DIR_PERMISSION);
    }
    //$thumbProfile = thumbnail($filename,$dirname,$dirname,300,300,false);
    $file = get_images2(G5_DATA_PATH."/profile/".$member["mb_no"]."/".$filename);
}*/
?>
<div class="sub_head">
...
<div id="settings" class="main">
	<div class="setting_wrap_top">
		...
		<div class="profile">
            ...
            <div class="profile_thumb" style="<?php if($member["mb_profile"]){?>background-image:url('<?php echo G5_DATA_URL."/profile/".$member["mb_no"]."/".$file;?>')<?php }else{ ?>background-image:url('<?php echo G5_IMG_URL?>/no-profile.svg')<?php }?>;background-size:cover;background-repeat:no-repeat;background-position:center;">
			<?php if($member["mb_profile"]){
			    ?>
				<img src="<?php echo G5_DATA_URL."/profile/".$member["mb_no"]."/".$file;?>" alt="" class="profile" id="profile_photo" style="display:none">
			<?php }else{ ?>
	...
	<div class="setting_wrap">
		...
		<ul>
            ...
            <?php if($passwdAuth==true){?>
			<li onclick="location.href='<?php echo G5_MOBILE_URL?>/page/mypage/password_settings.php?id=<?php echo $settings['id'];?>'">비밀번호 변경</li>
            <?php }?>
			<li onclick="location.href='<?php echo G5_MOBILE_URL?>/page/mypage/address_settings.php?id=<?php echo $settings['id'];?>'">주소변경 <span>배송시 필요</span></li>
		</ul>
	</div>
	<div class="setting_wrap">
		<h2>연락기능 설정</h2>
    ...
    <div class="setting_wrap">
        ...
        <ul>
            ...
            <li class="single" onclick="location.href='<?php echo G5_MOBILE_URL?>/page/mypage/my_bank.php?id=<?php echo $settings['id'];?>'">정산계좌등록</li>
        </ul>
    </div>
	<div class="setting_wrap">
		<h2>간편 거래 설정</h2>
    ...
    <div class="setting_wrap ">
        <h2>기기등록</h2>
        <ul>
            <li class="single" onclick="location.href='<?php /*echo G5_MOBILE_URL*/?>/page/mypage/device_add.php?id=<?php /*echo $settings['id'];*/?>'">연결기기등록</li>
        </ul>
    </div>
    --><?php /*}*/?>
    <?php if($is_member){?>
			...
			if(type=="show_hp"){
                ...
                if($(this).prop("checked")==true){
                    if($("#chat_set").prop("checked")==false){
                        ...
                        $("#hp_set").prop("checked",false);
                        $("#sms_set").prop("checked",false);
                        fnSetUpdate("hp_set",0);
                        fnSetUpdate("sms_set",0);
                        fnSetUpdate("show_hp",1);
                    }
                }else{
                    $("#hp_set").prop("checked",true);
                    $("#sms_set").prop("checked",true);
                ...
                if(hp_chk == sms_chk){
                    ...
                    if(hp_chk == false){
                        $("#show_hp").prop("checked",true);
                        fnSetUpdate("hp_set",0);
                        fnSetUpdate("sms_set",0);
                        fnSetUpdate("show_hp",1);
                    }else{
                        $("#show_hp").prop("checked",false);
                        fnSetUpdate("hp_set",1);
                        fnSetUpdate("sms_set",1);
                        fnSetUpdate("show_hp",0);
                    }
