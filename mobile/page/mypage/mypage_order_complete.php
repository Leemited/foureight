...
$sql = "select * from `order` as o left join `product` as p on o.pd_id = p.pd_id left join `g5_member` as m on p.mb_id = m.mb_id where o.od_fin_status = 1 and p.pd_status = 4 {$where} order by o.od_date desc";
$res = sql_query($sql);
while($row = sql_fetch_array($res)){
    $list[] = $row;
}
...
include_once (G5_MOBILE_PATH."/page/mypage/mypage.popup.php");
?>
...
<div id="mypage" <?php if($_SESSION["type1"]==2 || $_SESSION["type1"]==""){?>class="bg2"<?php }?>>
	...
	<section class="user_tab">
        ...
        <ul id="my_ul">
            ...
            <li class="myprofile wishes active">
                <div>거래완료</div>
        ...
        <ul class="order_cate">
            <li onclick="fnlist2(1,'3','','<?php echo $mb_id;?>','','1')" <?php if($od_cate==""||$od_cate==1){?>class="active"<?php }?>>판매</li>
            <li onclick="fnlist2(1,'3','','<?php echo $mb_id;?>','','2')" <?php if($od_cate==2){?>class="active"<?php }?>>구매</li>
        </ul>
        <!--<ul class="sub_ul">
    ...
    <section class="user_list">
        <article class="post" id="post">
            <div class="list_item grid" id="test">
                <?php
...
$(document).ready(function(){
    ...
    <?php //if($_REQUEST["type"]){?>
        ...
        fnlist2(1, 3, pd_type, "<?php echo $mb_id;?>",'','<?php echo $od_cate;?>');
    <?php //}?>
...
function fnlist2(num,type1,type,mb_id,stx,od_cate){
    ...
    var url = g5_url+"/mobile/page/ajax/ajax.mypage.order_confirm.php";
...
function fnTalk2(read_mb_id,pd_id,roomid,type,pd_type){
    ...
    var link = encodeURI(g5_url+"/mobile/page/mypage/mypage_order_complete.php?od_cate="+od_cate+"&pd_type="+pd_type);
