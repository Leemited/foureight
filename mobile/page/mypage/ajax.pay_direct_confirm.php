...
if($type==0){//판매자 요청 취소
    ...
    $sql = "update `order` set od_direct_status = -1,od_pay_type = 0, od_pay_status = 0 {$where} where od_id = '{$od_id}'";
    sql_query($sql);
    ...
    if($mb["regid"]){//기기정보 있을때
        send_FCM($mb["regid"], "48 직거래 알림", cut_str($order["pd_tag"],10,"...") . "의 직거래 요청이 취소되었습니다.\r\n전자결제를 통해 거래를 진행해 주세요.", G5_MOBILE_URL . "/page/mypage/mypage_order.php?type=2&od_cate=2&pd_type=".$order["pd_type"], 'fcm_buy_channel', '구매알림', $mb["mb_id"], $pd_id, $img,'','');
    }
    ...
    if($order["od_pd_type"]==1){
        $sql = "update `product` set pd_status = 1 where pd_id = '{$order["pd_id"]}'";
        sql_query($sql);
    }
    ...
    if(sql_query($sql)){
        ...
        if($mb["regid"]){//기기정보 있을때
            send_FCM($mb["regid"], "48 직거래 알림", cut_str($order["pd_tag"],10,"...") . "의 직거래 요청이 승인되었습니다.\r\n구매자와 직거래 장소와 시간을 확인 후 거래 바랍니다.\r\n직거래는 당사에서 책임을 지지 않으니 사기피해에 주의하세요.", G5_MOBILE_URL . "/page/mypage/mypage_order.php?type=2&od_cate=2&pd_type=".$order["pd_type"], 'fcm_buy_channel', '구매알림', $mb["mb_id"], $pd_id, $img);
        }

        alert("직거래 요청을 승인하였습니다.\\r구매자와 직거래 장소 및 시간을 최종 확인 바랍니다.",G5_MOBILE_URL."/page/mypage/mypage_order.php?type=2&od_cate=1");
    }else{
    ...
    if($mb["regid"]){//기기정보 있을때
        send_FCM($mb["regid"], "48 직거래 알림", cut_str($order["pd_tag"],10,"...") . "의 직거래가 취소되었습니다.\r\n전자결제를 통해 거래를 진행해 주세요.", G5_MOBILE_URL . "/page/mypage/mypage_order.php?type=1&od_cate=1&pd_type=".$order["pd_type"], 'fcm_buy_channel', '구매알림', $mb["mb_id"], $pd_id, $img);
    }
