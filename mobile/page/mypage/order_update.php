...
if(sql_query($sql)){
    ...
    if($mb["regid"]) {
        send_FCM($mb["regid"], '48결제알림', $order["pd_tag"]."의 결제가 완료 되었습니다.", G5_MOBILE_URL . "/page/mypage/mypage_order.php?od_cate=1&pd_type=".$order["pd_type"], 'fcm_buy_channel', "구매알림", $mb["mb_id"], $order["pd_id"], $img);
    }

    alert("결제 및 주문이 정상 처리되었습니다.",G5_MOBILE_URL.'/page/mypage/mypage_order.php?od_cate=2&pd_type='.$order["pd_type"]);
}else{
