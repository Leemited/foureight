...
if($od_cate){
    $back_url .= "?od_cate=".$od_cate."&pd_type=".$pd_type;
}
$set_type = $order["od_pd_type"];
...
?>
    ...
    <div class="alert_list">
        <div class="list_con">
            ...
            <?php if($order["od_delivery_type"]==1 || $order["pd_delivery_use"]==1){?>
            ...
            <?php }?>
            <?php if($order["delivery_name_cancel"]!=''){?>
                <div class="product_info" style="background-color:#fff;width: calc(100% - 8vw);padding: 2vw;margin: 2vw;">
                    <h2>환불 배송 정보</h2>
                    <ul>
                        <li>택배사 <span id="deli_name"><?php echo ($order["delivery_name_cancel"])?$order["delivery_name_cancel"]:"배송전";?></span></li>
                        <li>운송장번호 <span id="deli_num"><?php echo ($order["delivery_number_cancel"])?$order["delivery_number_cancel"]:"배송전";?></span></li>
                        <li>배송 등록일 <span id="deli_date"><?php echo ($order["delivery_cancel_date"])?$order["delivery_cancel_date"]:"배송전";?></span></li>
                    </ul>
                </div>
            <?php }?>
            <div class="order_view_btns">
                <!--<input type="button" value="확인" onclick="location.href='<?php /*if($back_url){echo $back_url;}else{*/?><?php /*echo G5_MOBILE_URL*/?>/page/mypage/mypage.php?type=2<?php /*}*/?>'">-->
                <input type="button" value="확인" onclick="location.href=g5_url+'/mobile/page/mypage/mypage_order.php?od_cate=2&pd_type=<?php echo $order["od_pd_type"];?>'">
                <?php if($order["delivery_name"] && $order["delivery_name_cancel"]==""){?>
                <input type="button" value="배송조회" onclick="fnDeliSearch('<?php echo $order["delivery_name"];?>','<?php echo $order["delivery_number"];?>');">
                <?php }?>
                <?php if($order["delivery_name_cancel"]){?>
                    <input type="button" value="환불배송조회" onclick="fnDeliSearch('<?php echo $order["delivery_name_cancel"];?>','<?php echo $order["delivery_number_cancel"];?>');">
                <?php }?>
            </div>
        </div>
    </div>
