...
if($mb["mb_id"]){
    ...
    $sql = "select * from `product_chat` where (send_mb_id = '{$member["mb_id"]}' and pd_id = '{$pd_id}') or (read_mb_id = '{$mb["mb_id"]}' and pd_id = '{$pd_id}') order by msg_datetime desc limit 0,1";
    $roomidchk = sql_fetch($sql);

    //if($roomid==""){
        $roomid = $roomidchk["room_id"];
    //}
    $result["roomchk"]=$roomidchk;
    ...
    if($set["chat_set"]=="1"){
        if($pd_id) {
            ...
            $result["obj"] .= "<li onclick=\"fnTalk2('".$mb["mb_id"]."','".$pd_id."','".$roomid."','".$type."','".$pd_type."')\"><img src='" . G5_IMG_URL . "/view_menu_talk.svg' alt=''><span>대화하기</span></li>";
        }else{
            $result["obj"] .= "<li onclick=\"fnTalk2('".$mb["mb_id"]."','".$pd_id."','".$roomid."','".$type."','".$pd_type."')\"><img src='" . G5_IMG_URL . "/view_menu_talk.svg' alt=''><span>대화하기</span></li>";
        }
    }
