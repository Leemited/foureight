        ...
        if($type2=="8") {
            if ($listadd[$k]["ad_sort"] == $addcnt) {
                ...
                <div class="grid__item ad_list <?php if ($list_type == "list") {echo " type_list";} ?>" onclick="location.href='<?php echo $listadd[$k]["ad_link"]; ?>'">
                    <div>
                        <div class="ad_mark">AD <?php echo $listadd[$k]["ad_sort"];?></div>
...
<div class="grid__item ajax_list <?php if($img1!=""){echo "images_list";}?> <?php if($flag){echo "wishedon";}?>  <?php if($list_type=="list"||$_SESSION["list_type"]=="list"){echo " type_list";}?> <?php if($list[$i]["pd_blind"]>=10){?>blinds<?php }?>" id="list_<?php echo $list[$i]['pd_id'];?>">
    ...
    <div class="in_grid">
		...
		<div class="top">
			<div>
                <h2 style="font-weight:normal"><?php echo ($list[$i]["mb_level"]==4)?"<img src='".G5_IMG_URL."/ic_pro.svg'>":"　";?><?php echo "<br><br>합계:".$list[$i]["sums"]."//최신:".$list[$i]["updates"]."//조회:".$list[$i]["hits"]."//거리:".$list[$i]["distances"]."//가격:".$list[$i]["prices"]."//추천:".$list[$i]["recoms"];?></h2>
				<div>
					<ul>
...
}else if(count($list)==0 && count($listadd)==0 && $page >= 1){echo "no-list";}
