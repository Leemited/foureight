...
<div class="wrap" style="height:calc(100vh - 29vw);">
    ...
    <?php }?>
	<div id="map" style="width:100%;<?php if($stx){?>height:calc(100vh - 20vw - 41vh);<?php }?>">
        <?php if($app || $app2){?>
...
if(lat && lng) {
    ...
    var positions = [
            ...
            ?>
        {
            ...
            latlng: new kakao.maps.LatLng('<?php echo $list[$i]["pd_lat"];?>','<?php echo $list[$i]["pd_lng"];?>'),
...
positions.forEach(function(pos){
    ...
    // 마커 이미지를 생성합니다
    if(pos.title=="현재위치") {
        markerImage = new kakao.maps.MarkerImage(imageSrc2, imageSize);
    }else {
...
function fnReset(){
    ...
    // 이동할 위도 경도 위치를 생성합니다
    var moveLatLon = new kakao.maps.LatLng(locs[0], locs[1]);
