...
if(ca_id){
    $today = date("Y-m-d");
    $sql = "select * from `write_confirm` where mb_id = '{$member["mb_id"]}' and cate_id = '{$ca_id}' and confirm_date = '{$today}'";
    $chk = sql_fetch($sql);
    if($chk==null){
            ...
            <form name="write_form" id="write_form" method="post" action="" onsubmit="return false">
                ...
                <div class="type2_box">
                    <label class="switch schtype2" >
                        <input type="checkbox" id="wr_type2" name="wr_type2" value="4" <?php if($pd_type2=="4"){?>checked<?php }?>>
                        <span class="slider round" <?php if($pd_type2=="4"){?>style="text-align: left" <?php }?>><?php if($pd_type2=="8"){?>판매<?php }else if($pd_type2=="4"){?>구매<?php }?></span>
                    </label>
                </div>
                <h2>검색어</h2>
                <div>
...
<script>
    ...
    $("#wr_title").click(function(){
        var ctl = document.getElementById("wr_title");
        var startpos = ctl.selectionStart;
        if(startpos==0){
            $(this).focus().setCursorPosition(1);
        }
    });
    ...
    $("#wr_title").keyup(function (e) {
        ...
        if(chk != "#"){
            if(($(this).val().charAt(0) > "가" || $(this).val().charAt(0) < "힣")){
                $(this).val("#");
            }
        }
        var cnt = text.split("#");
