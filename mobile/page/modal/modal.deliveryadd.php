...
$sql = "select od_content from `order` where od_id ='{$od_id}'";
$od = sql_fetch($sql);
...
<div id="id00" class="w3-modal w3-animate-opacity no-view">
    <div class="w3-modal-content w3-card-4">
        <div class="w3-container">
            ...
            <h2>배송정보 입력</h2>
            <div style="display: inline-block;margin: 4vw auto 0 auto;width:100%">
                <ul style="display:inline-block;margin:0 auto;">
                    <li style="background-color:rgba(255,255,255,0.6);padding:2vw;font-size:3vw;width:60vw;-webkit-border-radius: 5vw;-moz-border-radius: 5vw;border-radius: 5vw;"><span>배송요청사항 : </span> <?php echo ($od["od_content"]=="")?"요청사항 없음":$od["od_content"];?></li>
                </ul>
            </div>
            <div>
                ...
                <input type="number" value="" name="delivery_number" id="delivery_number" placeholder="운송장번호" required style="margin-top:0;" onkeyup="number_only(this)">
            </div>
            <div>
                <input type="button" value="취소" onclick="modalClose(this)"><input type="button" value="배송정보 등록" onclick="fnConfirmDelivery('<?php echo $od_id;?>','<?php echo $pd_id;?>','<?php echo $pd_type;?>','<?php echo $status;?>');" style="width:auto;margin-left:1vw" >
            </div>
        </div>
    </div>
</div>
