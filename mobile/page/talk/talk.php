...
<div id="container" style="background-color:#e7e7e7;height:calc(100vh - 28vw);overflow-y:scroll">
    ...
    ?>
        <div class="talk_item <?php if($mb["mb_leave_date"]){?>disable<?php }?>" <?php if($mb["mb_leave_date"]==""){?>onclick="fnTalkView('<?php echo $my_talk[$i]['pd_id'];?>','<?php echo $my_talk[$i]['send_mb_id'];?>','<?php echo $my_talk[$i]['read_mb_id'];?>','<?php echo $img1;?>','<?php echo $my_talk[$i]['room_id'];?>');"<?php }else{?>onclick="alert('해당 대화는 상대가 탈퇴하여 접근 할 수 없습니다.');"<?php }?> >
            <?php 
            if(is_file(G5_DATA_PATH."/product/".$img1)){ ?>
            <div class="pd_image" style="background-image:url('<?php echo G5_DATA_URL."/product/".$img1?>');background-size:cover;background-repeat:no-repeat;background-position:center;-webkit-border-radius:50% ;-moz-border-radius:50% ;border-radius: 50%;">
            </div>
            <?php }else{?>
