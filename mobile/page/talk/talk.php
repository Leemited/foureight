...
$sql = "select *,p.mb_id as pd_mb_id from `product_chat` as c left join `product` as p on p.pd_id = c.pd_id where send_mb_id = '{$member['mb_id']}' or read_mb_id = '{$member['mb_id']}' group by c.room_id order by msg_datetime desc";
$res = sql_query($sql);
...
<div id="container" style="background-color:#e7e7e7;height:calc(100vh - 30vw);overflow-y:scroll">
    ...
    $mb = get_member($my_talk[$i]["pd_mb_id"]);
    ?>
