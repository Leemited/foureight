...
}else if($return_url!="mypage" && $return_url!=""){
    $back_url = $return_url . "&pd_type=" . $pd_type;
}else{
    $back_url = G5_MOBILE_URL . "/page/talk/talk.php";
}
if($roomid){
    $room = " and room_id = '{$roomid}'";
}

$sql = "select *,c.id as id,p.mb_id as mb_id from `product` as p left join `product_chat` as c  on p.pd_id = c.pd_id where (c.pd_id = {$pd_id} or p.pd_id ='{$pd_id}') {$room} order by msg_datetime asc";
$res = sql_query($sql);
...
if(!$read_mb_id || $read_mb_id==""){
    $read_mb_id = $_REQUEST["read_mb_id"];
}

if($read_mb_id){
    $mbs = get_member($read_mb_id);
    if($mbs["mb_leave_date"]){
        alert("탈퇴한 회원입니다.");
    }
}
...
<div class="talk talk_view_container <?php if($talk_list[0]["pd_type"]==2 || $pd_type==2){?>bg2<?php }?>" style="<?php if($img1!=""){?>background-image:url('<?php echo G5_DATA_URL."/product/".$img1;?>')<?php }else{?>background-color:<?php echo $bg[$rand];  }?>;background-size:cover;background-repeat:no-repeat;background-position:center;">
    <?php if($type!="payment"){?>
    ...
    <div class="price">
        ...
        <h2><?php if(($talk_list[0]["pd_price"]+$talk_list[0]["pd_price2"])>0){echo "￦ ".number_format($talk_list[0]["pd_price"]+$talk_list[0]["pd_price2"]);}else{echo "무료나눔";}?></h2>
        <?php if($talk_list[0]["pd_type"]==1 && $talk_list[0]["pd_type2"]==8 && $talk_list[0]["mb_id"] == $member["mb_id"]){?>
            <div class="sell_btn">
                <?php if($btn==true){?>
                <input type="button" value="결제요청하기" class="btn" onclick="fnSell('<?php echo $talk_list[0]["pd_price"]+$talk_list[0]["pd_price2"];?>','<?php echo $talk_list[0]["pd_type"];?>');">
                <?php }else{?>
                <input type="button" value="결제대기중" class="btn" disabled>
                <?php }?>
            </div>
        <?php }?>
        <?php if($type=="order_fin" && $pd_type == 2 && $pro["mb_id"]!=$member["mb_id"]){?>
            <div class="sell_btn">
                <input type="button" value="재구매요청" class="btn" onclick="fnSell2('<?php echo $talk_list[0]["pd_price"];?>','<?php echo $talk_list[0]["pd_price2"];?>','<?php echo $pd_id;?>')">
            </div>
        <?php }?>
    ...
    <div class="msg_container" id="msg_container">
        ...
        <?php }else {?>
            ...
            for ($i = 0; $i < count($talk_list); $i++) {
                if($talk_list[$i]["message"]==""){continue;}
    ...
    <div class="msg_controls">
        ...
        <input type="hidden" value="<?php echo $read_mb_id;?>" id="read_mb_id">
        <input type="hidden" value="<?php echo $roomid;?>" id="roomid">
...
<script>
    ...
    function addSell(){
        ...
        if(confirm("해당 판매글의 상태가 판매중으로 변경됩니다.\r판매 하시겠습니까?")) {
            //바로 지목 판매이므로 상태는 판매중으로 변경 //물건인경우
            $.ajax({
                url: g5_url + "/mobile/page/ajax/ajax.insert_cart.php",
                method: "POST",
                data: {price: price, pd_id: pd_id, sell_mb_id: sell_mb_id, status: 1,addtype:'confirm'}
            }).done(function (data) {
                if(data=="3"){
                    fnSendMsSimple("판매자님께서 결제를 요청하셨습니다.<br>거래를 원하시면 마이페이지->거래진행중에서 결제를 진행해 주세요.");
    ...
    function fnSell2(price,price2,pd_id){
        ...
        if(confirm("예약 하시겠습니까?\r\n예약 후에는 판매자가 승인을 하면 해당 거래에 대한 결제가 진행됩니다.")) {
            $.ajax({
                url: g5_url + "/mobile/page/ajax/ajax.insert_cart.php",
                method: "POST",
                data: {price: price, price2: price2, pd_id: pd_id, status: 0, pd_type: 2}
            }).done(function (data) {
                if (data == "3") {
