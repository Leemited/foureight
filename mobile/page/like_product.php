...
if($prochk["pd_type"] == 2) {
    $sql = "insert into `product_like` set pd_id='{$pd_id}', mb_id='{$mb_id}', like_content='{$like_content}', like_date = now(), pd_type = 2";
    if (sql_query($sql)) {
    ...
    if(sql_query($sql)){
        $sql = "insert into `product_like` set pd_id='{$pd_id}', mb_id='{$mb_id}', like_content='{$like_content}', like_date = now(), pd_type = 1";
        sql_query($sql);
