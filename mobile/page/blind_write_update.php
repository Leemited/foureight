...
if($myblind["cnt"] > 0){
    alert("이미 신고한 게시물입니다.",G5_MOBILE_URL."/page/view.php?pd_id=".$pd_id);
    return false;
}

$sql = "insert into `product_blind` set mb_id= '{$mb_id}', pd_id='{$pd_id}', blind_content = '{$blind_con}', blind_date = now()";


if(sql_query($sql)){
    $sql = "update `product` set pd_blind = pd_blind + 1 where pd_id ='{$pd_id}'";
    sql_query($sql);
    alert("신고처리 되었습니다. 신고는 게시물당 한번만 할 수 있으며 누락 10건일 경우 자동 블라인드 처리됩니다.",G5_MOBILE_URL."/page/view.php?pd_id=".$pd_id);
}else{
