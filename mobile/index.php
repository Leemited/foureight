			...
			<div class="list_item grid are-images-unloaded" id="test">
				...
				for($i=0;$i<count($list);$i++){
                    ...
                    <div class="grid__item ad_list <?php if($_SESSION["list_type"]=="list"){echo " type_list";}?>" onclick="location.href='<?php echo $listadd[$k]["ad_link"];?>'">
                        <div>
                            <div class="ad_mark">AD<?php echo $addcnt;?></div>
                            <?php if($listadd[$k]["ad_photo"]!=""){
                                ?>
                                <div class="item_images" style="background-repeat:no-repeat;background-size:cover;background-position:center;min-height: 28vw;background-image:url('<?php echo G5_DATA_URL;?>/product/<?php echo $listadd[$k]["ad_photo"];?>')" >
				...
				<div class="grid__item <?php if($flag){echo "wishedon";}?> <?php if($_SESSION["list_type"]=="list"){echo " type_list";}?> <?php if($list[$i]["pd_blind"]>=10){?>blinds<?php }?>" id="list_<?php echo $list[$i]['pd_id'];?>">
					...
					<div class="in_grid">
                        <?php if($list[$i]["pd_images"]!=""){
                            ...
                            $img[0] = trim($img[0]);
                            $img1 = get_images(G5_DATA_PATH."/product/".$img[0],500,'');
						...
						<div class="top">
							<div>
								<h2 style="font-weight:normal"><?php echo ($list[$i]["mb_level"]==4)?"<img src='".G5_IMG_URL."/ic_pro.svg'>":"　";?><?php echo "<br><br>합계:".$list[$i]["sums"]."//".$list[$i]["updates"]."//".$list[$i]["hits"]."//".$list[$i]["distances"];?></h2>
								<div>
									<ul>
...
/*								if($flag){
				...
				if(count($list)==0 && count($listadd)>0 && $page == 1){
				    for($k=0;$k<count($listadd);$k++){?>
                    <div class="grid__item ad_list <?php if($_SESSION["list_type"]=="list"){echo " type_list";}?>" onclick="location.href='<?php echo $listadd[$k]["ad_link"];?>'">
                        <div>
                            ...
                            <?php if($listadd[$k]["ad_photo"]!=""){
                                ?>
                                <div class="item_images" style="background-image:url('<?php echo G5_DATA_URL?>/product/<?php echo $listadd[$k]["ad_photo"];?>');background-repeat:no-repeat;background-size:cover;background-position:center;min-height: 28vw;">
                                    <img src="<?php echo G5_DATA_URL?>/product/<?php echo $listadd[$k]["ad_photo"];?>" alt="ad" class="main" style="opacity:0">
                                </div>
                            <?php  }?>
                            <div class="bottom" >
                    ...
                    <?php }
                }else if(count($list)==0 && count($listadd)==0){?>
                    <div class="no-list">
                        검색된 리스트가 없습니다.
                    </div>
                <?php }?>
...
$(document).ready(function(){
	...
	$(".category_menu .category2 ul li, .category_menu2 .category2 ul li").click(function(){
        ...
        /*$.ajax({
            ...
            method:"post",
            data:{set_type:$("#set_type").val(),cate1:c,cate2:sc,pd_type2:"<?php echo $pd_type2;?>",app:"<?php echo $app;?>",app2:"<?php echo $app2;?>"}
        }).done(function(data) {
                ...
                $.ajax({
                    ...
                    if(data.info_text) {
                        ...
                        $.ajax({
                            ...
                            method: "post",
                            data: {infotext: info_text,ca_id: data.ca_id,cate1: c,cate2: sc,set_type: $("#set_type").val(),pd_type2: "<?php echo $pd_type2;?>",app: "<?php echo $app;?>",app2: "<?php echo $app2;?>",catetag: data.catetag,ca_id:data.ca_id}
                        }).done(function (data) {
                            $(".modal").html(data).addClass("active");
                            $("html, body").css("overflow", "hidden");
                            $("html, body").css("height", "100vh");
                    ...
                    /*$.ajax({
                        ...
                        method: "post",
                        data: {cate1: c, cate2: sc,set_type:$("#set_type").val(),pd_type2:"<?php echo $pd_type2;?>",app:"<?php echo $app;?>",app2:"<?php echo $app2;?>"},
                        dataType: "json"
                    }).done(function (data) {
                        console.log(data);
                        if (data.catetag != "") {
                            $("#id01 .write_help").html("예 : " + data.catetag);
                            if (type == 2) {
                                $("#id01 .write_help.price_help").html("필요시 계약금을 설정할 수 있습니다.");
                            }
                        } else {
                        ...
                        if (type2 == true) {
                            if (data.info_text1) {
                                $.ajax({
                                    url:g5_url+'/mobile/page/modal/modal.writeconfirm.php',
                                    method:"post",
                                    data:{infotext:data.info_text,ca_id:data.ca_id}
                                }).done(function(data){
                                    $(".modalup").html(data).addClass("active");
                                    //$(".modal").hide();
                                });
                            }
                        } else {
                            if (data.info_text2) {
                                $.ajax({
                                    url:g5_url+'/mobile/page/modal/modal.writeconfirm.php',
                                    method:"post",
                                    data:{infotext:data.info_text2,ca_id:data.ca_id}
                                }).done(function(data){
                                    $(".modalup").html(data).addClass("active");
                                    //$(".modal").hide();
                                });
...
function fnlist(num,fn_list_type){
	...
	$.ajax({
		...
		method:"POST",
		data:{page:page,list_type:set_list_type,stx:stx,app:app,app2:app2,set_type:type1,type2:type2,cate1:cate1,cate2:cate2,priceFrom:priceFrom,priceTo:priceTo,sc_id:sc_id,mb_id:mb_id,order_sort:align,latlng:latlng,pd_ids:pd_ids,mb_level:typecompany,order_sort_active:orderactive,set_search:set_search,pd_price_type1:price_type1,pd_price_type2:price_type2,pd_price_type3:price_type3,pd_timeFrom:meetFrom,pd_timeTo:meetTo,pd_timeType:meetType,lat:"<?php echo $_SESSION["lat"];?>",lng:"<?php echo $_SESSION["lng"]?>",searchs:searchs,searchActive:searchActive},
		beforeSend:function(){
		    <?php if($app){?>
...
$(window).scroll(function(){
    //console.log(Math.ceil($(this).scrollTop()) + "//" + ($(document).height() - $(window).height()));
	if (Math.ceil($(this).scrollTop()) >= ($(document).height() - $(window).height())) {
	    //if(location.hash == "") { //막앗던 이유 파악
