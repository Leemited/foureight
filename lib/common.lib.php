...
function string_star($str,$cut_len=3,$type='right',$star="*"){
    ...
    if($type == "right"){
        $temp = mb_substr($str,0,$length - $cut_len,'utf-8');
        for($i=0;$i<$cut_len;$i++){
            $temp.=$star;
        }
        $ref = $temp;
    }else if($type=="left"){
        $temp = mb_substr($str,$cut_len,$length - $cut_len,'utf-8');
        $temp2 = "";
        for($i=0;$i<$cut_len;$i++){
            $temp2 .= $star;
        }
        ...
        $harf = floor($length/2);
        $harf_cut_len_1 = floor($cut_len/2);
        $harf_cut_len_2 = ceil($cut_len/2);

        $temp2 = "";

        $temp_head = mb_substr($str,0,$harf-$harf_cut_len_1,'utf-8');
        $temp_foot = mb_substr($str, $harf+$harf_cut_len_2, $harf,'utf-8');
